...
function populateNav(starships) {
    starships.forEach(starship => {
        ...
        shipAnchor.addEventListener('click', event =>  {
            //store name of list item clicked on
...
function populateShipView(shipData) {
    ...
    shipImage.src = `http://starwars-visualguide.com/assets/img/starships/${imageNum}.jpg`


shipImage.addEventListener('error', event => {
    shipImage.hidden = true
    modalDialog.classList.toggle("is-active")
    modalMessage.textContent = `Sorry, cound not find an image for ${shipData.name}.`
})
